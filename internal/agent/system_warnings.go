...
func CheckSystemWarnings() []string {
	...
	// Build map of disabled checks (from config or SUPPRESS_WARNINGS env var)
	...
	// 1. Analyze active tool requirements across all loaded agent blueprints
	...
	for _, custom := range cfg.CustomChecks {
		...
		// Skip custom check if required tool is specified but not assigned to any active agent
		if custom.RequiredTool != "" && !activeTools[ResolveToolAlias(custom.RequiredTool)] {
			continue
		}
